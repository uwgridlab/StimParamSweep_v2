...
%% To avoid a long wait for the first question window to appear
% On 7/23/18 had trouble getting the first question window to appear
% quickly. It was taking 12-18 sec rather than ~20 ms. Not sure why, but
% this is a work around to create a text window with just a space before
% starting any of the stimulation.

% Wait for the "READY to Stim again!" button to turn green to start the
% experiment
...
disp('Ready to start experiment')
...
% changed this to 1.5 sec on 7/23/18 (the below, in for loop, was already
% set to 1.5)
...
%     disp('yes no question on')
    ...
    % Until we've reached the last trial, keep triggering the TDT to
    % deliver another stim train
    if i<numTrials
...
vbl = displayFixationCross(window, white, xCenter, yCenter);
        ...
        DA.SetTargetVal('RZ5D.yesButton', 0); % turn yes button back off
        DA.SetTargetVal('RZ5D.noButton', 0); % turn no button back off
